...
```{r AtletesFigure, echo=T, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "\\label{fig:Atlevar} Datos de atletas. Densidad de la hemoglobina para hombres y mujeres (derecha) y diagrama de dispersión entre la homoglobina y el índice de masa corporal (izquierda). Negro para hombres y rojo para mujeres.",warning=FALSE,message = FALSE}
data(ais)
par(mfrow=c(1,2))
...
```
En la Figura \@ref(fig:Atlevar) (izquierda) vemos que los niveles de hemoglobina son mayores para hombres que para mujeres. En la Figura \@ref(fig:Atlevar) (derecha) observamos que hay una relación positiva entre la hemoglobina y el índice de masa corporal tanto para hombres como para mujeres. Esto nos puede indicar que ingresar el sexo del atleta en el modelo puede mejorarnos el ajuste.
...
```{r UN11Figure, echo=T, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "\\label{fig:UN112var} Datos de la ONU. Relación entre la fertilidad y el PNB per cápita para los países de OCDE (puntos negros), países africanos (puntos verdes) y los otros países (rojos)",warning=FALSE,message = FALSE}
data(UN11)
...
\]
Por lo que el modelo (\@ref(eq:modInter)) genera dos rectas, una para cada categoría. $\beta_2$ indica la diferencia de intercepto entre las dos categorías y $\beta_3$ la diferencia entre pendientes. En la Figura \@ref(fig:Rectas)(izquierda) se observan las dos rectas que se obtienen a partir de este modelo. Si se elimina la interacción entre variables, se obtienen dos rectas paralelas (Figura \@ref(fig:Rectas), derecha).


```{r Rectas1,echo=FALSE, fig.height = 4, fig.width = 9,fig.align = "center",fig.cap = "\\label{fig:Rectas} Efecto de la interacción entre variable continua e indicadora. Modelo general (izquierda) y modelo de líneas paralelas (derecha)."}
par(mfrow=c(1,2))
...
```{r AtletesAjuste, echo=T, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "\\label{fig:AtleAjuste1} Datos de atletas. Ajuste del modelo para la homoglobina en función  del índice de masa corporal y sexo. Línea negra para hombres y línea roja para mujeres.",warning=FALSE,message = FALSE}
plot(Hg~BMI,data=ais,col=ais$Sex+1,ylab='Hg (g/dl)',xlab='BMI')
...
summary(mod.ais.lp)
```
La estimación del efecto asociado al sexo indica la diferencia media del nivel de hemoglobina entre hombres y mujeres. En la Figura \@ref(fig:AtleAjuste2) vemos que el modelo de líneas paralelas tiene casi el mismo ajuste que el modelo general.

```{r AtletesAjuste2, echo=T, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "\\label{fig:AtleAjuste2} Datos de atletas. Ajuste del modelo general (línea solida) y el modelo líneas paralelas (línea discontinua) para la homoglobina en función  del índice de masa corporal y sexo. Líneas negra para hombres y líneas roja para mujeres.",warning=FALSE,message = FALSE}
plot(Hg~BMI,data=ais,col=ais$Sex+1,ylab='Hg (g/dl)',xlab='BMI')
abline(a=mod.ais$coefficients[1],b=mod.ais$coefficients[3],lwd=2)
abline(a=mod.ais$coefficients[1]+mod.ais$coefficients[2],b=mod.ais$coefficients[3]+mod.ais$coefficients[4],col=2,lwd=2)
...
\]
Aquí vemos que para los países que no son de la OCDE, el producto nacional bruto tiene un efecto significativo negativo sobre la tasa de fertilidad. Mientras que para los países de la OCDE, este efecto es positivo, aunque no es significativo.

Esto mismo lo podemos ver gráficamente en la Figura \@ref(fig:MCOUN11).
```{r MCOUN11Figure, echo=T, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "\\label{fig:MCOUN11} Datos de la ONU. Ajuste del modelo para la fertilidad en función  del PNB y tipo de país. Países de OCDE (línea negra), países africanos (línea verde) y los otros países (línea roja)",warning=FALSE,message = FALSE}
