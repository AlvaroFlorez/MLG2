...
```
Vemos que se rechaza $H_0$, es decir que hay diferencias en los niveles de homoglobina entre hombres y mujeres. $\beta_1$ o $\beta_3$ es diferente de cero, pero no ambos (esto por los resultados del modelo completo).

Eliminando la interacción obtenemos un modelo con lineas paralelas:
\[
...
- El efecto del PNB per cápita sobre la fertilidad depende del tipo de país.
- Para los países miembros de la OECD, no hay un efecto significativo.
- Sin embaro, para los demás países, el efecto es negativo (y no es significativamente diferente para las categorías de África y otros).