...
```{r, include=TRUE, fig.align='center',fig.cap="Fredericia(línea negra), Horsens(línea roja), Kolding(línea verde), Vejle(línea azul)."}
...
```{r, include=TRUE, fig.align='center',fig.cap="Histograma(izquierda), número de satélites vs peso (derecha)"}
par(mfrow=c(1,2))
...
```{r,include=TRUE, fig.align='center',fig.cap="Fredericia (línea negra), otra ciudad (línea roja)."}
...
knitr::include_graphics(here::here("figs", "DistribucionNegBin.jpg"))
```
...
mathy.df <- data.frame(b0=c("Poisson","Binomial-negativa"), 
                       b1=c(149.68,70.124),
                       b2=c(56,56),
                       b2=c(343.44,313.32))

colnames(mathy.df)<-c("Modelo","Devianza","g.l.","AIC")
...
## Modelo Poisson con ceros inflados

El modelo Poisson con ceros inflados (ZIP):
...
```{r, echo=FALSE,fig.align='center', fig.cap="", out.width = '75%'}
knitr::include_graphics(here::here("figs", "DistribucionZIP.jpg"))
```
