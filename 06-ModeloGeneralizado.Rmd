...
$$
Por lo  que $\by \sim N(\bX\bbeta, \sigma^{2}\bI)$. Esto implica que:
...
* ```base```: número de ataques epilépticos ($\times$8 semanas) antes de iniciar los tratamientos.

* ```treatment```: tratamiento (placebo, Progabida).

* ```seizure.rate```(variable respuesta): número de ataques epilépticos ($\times$2 semanas) luego de 8 semanas.
...
library(HSAUR2)
data(epilepsy)
epilepsy4=subset(epilepsy, period==4)#selecionamos la semana 4
...
La distribución normal también pertenece la familia exponencial. La función de densidad:
...
Finalmente, también la distribución binomial también pertenece a la familia exponencial. La función densidad:
...
$$
Por lo cual, es MLE es un estimador eficiente (Teorema de Cramer-Rao). 
...
* Algoritmo para el ajuste del modelo.
...
\end{cases}
\]
Por lo que, para $i$-ésima dosis, el número de escarabajos que muere es igual a $\sum_{j=1}^{n_i} y_{ij}$. Además, asumiendo independencia, podemos decir que:
$$
\sum_{j=1}^{n_i} y_{ij} \sim \mbox{binomial} (n_{i},\pi_{i}), i=1,\ldots,n_i,
$$
donde $0 < \pi_i < 1$ depende de la dosis. Definiendo $y_{i}=\sum_{j=1}^{n}y_{ij}/n_{i}$ (la proporción de escarabajos muertos para la dosis $i$), tenemos que:
$$
E(y_{i}| \bx_{i}) = \pi_{i} \mbox{ y }V(y_{i}| \bx_{i}) = \pi_{i}(1-\pi_{i})/n_{i}.
$$

Aquí estamos interesados en modelar $\pi_i$ en función de la dosis. Dado que $0 \leq \pi_i \leq 1$, se podría proponer que:
$$
...
Definiendo $y_i$ como el número de ataques epilépticos ($\times$ dos semanas) luego de 8 semanas del paciente $i$, podemos suponer que:
$$
...
Por lo que, $g(\lambda_{i})=log\lambda_{i}=\bx_{i}^{'}\bbeta$. La función de enlace escogida es logarítmica.  Este modelo puede ser una opción para estimar el número medio de ataques epiléticos de los pacientes del ensayo clínico en epilepsia. 
...
```{r escarabajosEstFig, fig.height = 4, fig.width = 6,fig.align = "center",fig.cap = "Datos de escarabajos. Estimación de la probabilidad de que el escarabajo muera en función del logarítmo de la dosis"}
...
$$
Es decir, es una razón entre la probabilidad de éxito sobre la probabilidad de fracaso. Por ejemplo, si el odd es igual a dos, estaríamos diciendo que la probabilidad de éxito es dos veces mayor a la probabilidad de fracaso. Para el caso del modelo logístico simple, tenemos que:
...
Para los datos de los escarabajos un cambio unitario en el log dosis es muy grande, por lo que podríamos interpetra $\beta_1$ usando un aumento en la log dosis de $\delta=0.02$:
...
En el modelo Poisson, tenemos que: $E(Y|x)=\exp(\beta_0+x\beta_1)$. Si aumentamos a $x$ en $\delta$ unidades, tenemos que $E(Y|x+\delta)=\exp(\beta_0+(x+\delta)\beta_1)$. Ahora si calculamos el logaritmo de la razón de los valores esperados obtenemos que:
...
$$
donde $\bx_{1 }= (1,x_{11},x_{12},\ldots,,x_{1,q-1})'$ y $\bx_{2}= (1,x_{21},x_{22},\ldots,,x_{2,p-q})'$ con vectores de covariables; $\bbeta_1$ y $\bbeta_2$ vectores de coeficientes de dimiensiones $q$ y $p-q$. Ahora, planteamos el siguiente sistema de hipótesis:
$$
...
\qquad H_0:\beta_1=\beta_3=0.
$$
Es decir, no hay ningún efecto del tratamiento para la epilépsia (no es estadísticamente diferente al placebo). La prueba de razón de verosimilitud se puede implementar de las siguiente forma:
...
```
Este resultado sugiere que hay un efecto del tratamiento. Es decir, el Progabida tiene un efecto significativo para reducir los ataques epilépticos. 
...
```
La prueba de Wald proporciona resultados similares a la de razón de verosimilitud y llegamos a la misma conclusión.
...
La media del número de ataques epiléptico por 2 semanas se puede observar en la Figura \@ref(fig:grafICepilep). Se puede observar que las estimaciones de la media para el grupo tratamiento es inferior que para el grupo control.

```{r grafICepilep, fig.height = 4, fig.width = 6,fig.align='center',fig.cap="Datos de epilépsia. Estimación de la media de ataques epilépticos por 2 semanas en el grupo placebo(negro) y el grupo tratamiento(rojo)."}
...
$$
D=2\sum_{i=1}^n\left[y_ilog \left(\frac{y_i}{\widehat{\mu}_i}\right)+(n_i-y_i)log\left(\frac{n_i-y_i}{n_i-\widehat{\mu}_i}\right)\right],
$$
...
$$
D=2\sum_{i=1}^ny_ilog\left(\frac{y_i}{\widehat{\mu}_i}\right), \quad \text{donde} \quad \widehat{\mu}_i=\widehat{\lambda}_i.
$$
...
$$
H_0:\mbox{logit}(\pi_i)=\beta_0+\sum_{j=1}^{p-1}\beta_j x_{ij}.
$$
