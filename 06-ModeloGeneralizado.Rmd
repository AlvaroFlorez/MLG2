...
\[
y_{ij} = \begin{cases}
1 & \mbox{ si el escarabajo \textit{j} expuesto a la dosis \textit{i} muere,} \\
...
\]
donde $0 < \pi_i < 1$ depende de la dosis. Definiendo $y_{i}=\sum_{i=1}^{n}y_{ij}/n_{i}$ (la proporción de escarabajos muertos para la dosis $i$), tenemos que:
\[
...
$$
y_{i}\sim Poisson(\lambda_{i}), \quad i=1,...,n,\quad \text{donde} \quad \lambda_{i}=g(\eta_i),
$$
...
#### Otros ejemplos
Un modelo lineal se puede representar como un GLM. Aquí asumimos que $y_i \sim N(\mu_i, \sigma^2)$, con $\mu_i = \bx_i'\bbeta$. Es decir, la función de enlace es la identidad. Otros ejemplos de distribuciones que se pueden utilizar son:
...
### Método de razón de verosimilitud
La idea de este método es comparar la verosimilitud bajo dos condiciones: ($\ell_0$) la máxima alcanzada bajo $H_0$ (es decir, asumiendo que $\bbeta_2=\bZERO$) y ($\ell_1$) la evaluada en la estimación maximo verosimil, es decir $L(\hatbbeta)$. El estadístico de prueba usado es:
$$
...
La estimación de la probabilidad de que el escarabajo muera se puede observar en la Figura \@{fig:grafICesc}.
