...
\begin{equation}
\hat{\btheta}=\btheta^*+[U(\btheta^*)^{'}U(\btheta^*)]^{-1}U(\btheta^*)^{'} \hat{e^*}
(\#eq:AlgorGN)
\end{equation}
...
```{r tresmodelospavos, include=TRUE, fig.align='center', fig.cap="Datos Pavos.Modelo 1(Rojo), Modelo logístico(verde) y Modelo Weibull(Azul) "}
...
```{r linealpuromicina, include=TRUE, fig.align='center', fig.cap="Datos puromicina. Diagrama de dispersión. Datos linealizados(derecha)."} 
par(mfrow=c(1,2))
...
$$
1/y_{i}=\beta_{0}+1/conc_{i}\beta_{1}+\epsilon_{i}
$$
...
```{r puromicinaplotl, include=TRUE, fig.align='center', fig.cap="Datos puromicina. Enzimas tratadas(negro), enzimas no tratadas(rojo). "}
